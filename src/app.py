...
def show_landing_page():
    ...
    with col1:
        ...
        - **IPC分類・キーワードの推定**  
          内容に応じて、関連するIPC（国際特許分類）やキーワードを自動で推定します。
    ...
    with col2:
        ...
        - **検索SQLの自動生成**  
          推定されたIPCやキーワードに基づき、BigQuery用のSQLを自動生成します。
    ...
    with col3:
        ...
        - **Embeddingによるベクトル化**  
          タイトル・要約文・調査テーマを数値ベクトルに変換します。

        - **コサイン類似度によるスコアリング**  
          調査テーマと各特許文献とのベクトルの内積を計算し、関連度を定量評価します。

        - **関連性順での並び替え**  
          類似度スコアが高い順に特許をソートし、表示します。
        """)
